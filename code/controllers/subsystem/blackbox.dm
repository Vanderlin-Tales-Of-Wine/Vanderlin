...
/datum/controller/subsystem/blackbox/fire()
	...
	if(CONFIG_GET(flag/use_exp_tracking))
		if((triggertime < 0) || (world.time > (triggertime + 5 MINUTES)))	//subsystem fires once at roundstart then once every 10 minutes. a 5 min check skips the first fire. The <0 is midnight rollover check
			update_exp(10)
...
/datum/controller/subsystem/blackbox/proc/ReportDeath(mob/living/L)
	...
	if(!L.suiciding && !first_death.len)
		...
		first_death["role"] = !is_unassigned_job(L.mind?.assigned_role) ? L.mind.assigned_role.title : null
