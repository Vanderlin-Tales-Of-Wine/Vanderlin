		...
		if(is_cleanable(A))
			...
			if(cleaned_human.body_position == LYING_DOWN)
				...
				if(cleaned_human.wear_armor)
					SEND_SIGNAL(cleaned_human.wear_armor, COMSIG_COMPONENT_CLEAN_ACT, CLEAN_STRENGTH_BLOOD)
				else if(cleaned_human.wear_pants)
					SEND_SIGNAL(cleaned_human.wear_pants, COMSIG_COMPONENT_CLEAN_ACT, CLEAN_STRENGTH_BLOOD)
