...
/datum/enchantment/silver/on_hit(obj/item/source, atom/target, mob/user, proximity_flag, click_parameters)
	if(source in last_used)
		if(world.time < src.last_used[source] + 100)
			return
	. = ..()
	if(ishuman(target))
...
/datum/enchantment/silver/on_equip(obj/item/i, mob/living/user)
	. = ..()
	if(ishuman(user))
...
/datum/enchantment/silver/on_pickup(obj/item/i, mob/living/user)
	var/mob/living/carbon/human/H = user
	if(H.mind)
		var/datum/antagonist/vampire/V_lord = H.mind.has_antag_datum(/datum/antagonist/vampire/)
		var/datum/antagonist/werewolf/W = H.mind.has_antag_datum(/datum/antagonist/werewolf/)
