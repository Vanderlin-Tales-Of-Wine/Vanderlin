/datum/test_situation/hair_examine
	start = "hair_examine"
...
/datum/test_situation/hair_examine/execute_test(obj/abstract/visual_ui_element/scrollable/console_output/output, list/arg_list)
	. = ..()
	...
	for(var/datum/sprite_accessory/hair/head/hair as anything in subtypesof(/datum/sprite_accessory/hair/head))
		if(is_abstract(hair))
			continue
		step++
		if(step > 13)
