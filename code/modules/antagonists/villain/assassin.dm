...
/datum/antagonist/assassin/on_gain()
	...
	if(owner.current.job != "Drifter") // This code only runs if the assassin is assigned midround and is not a drifter.
		...
		owner.current.change_stat(STATKEY_SPD, 4)
		...
		if(old_knife_skill < 6) // If the assassined player has less than 4 knife skill, get them to 4.
			owner.current.mind.adjust_skillrank(/datum/skill/combat/knives, 6 - old_knife_skill, TRUE)
		if(old_sneak_skill < 6) // If the assassined player has less than 5 sneak skill, get them to 5.
			owner.current.mind.adjust_skillrank(/datum/skill/misc/sneaking, 6 - old_sneak_skill, TRUE)
