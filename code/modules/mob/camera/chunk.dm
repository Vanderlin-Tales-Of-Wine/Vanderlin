...
/datum/camerachunk/proc/remove(mob/camera/eye, remove_static_with_last_chunk = TRUE)
	eye.visibleCameraChunks -= src
	seenby -= eye
...
/datum/camerachunk/proc/update()
	...
	for(var/camera in cameras)
		var/obj/structure/fake_machine/camera/c = camera
...
/datum/camerachunk/New(x, y, z)
	...
	for(var/obj/structure/fake_machine/camera/c in urange(CHUNK_SIZE, locate(x + (CHUNK_SIZE / 2), y + (CHUNK_SIZE / 2), z)))
		if(c.can_use())
			cameras += c
