...
/mob/living/update_sneak_invis(reset = FALSE) //Why isn't this in mob/living/living_movements.dm? Why, I'm glad you asked!
	...
	if(sneaking) //If sneaking, check if they should be revealed
		if((stat > SOFT_CRIT) || IsSleeping() || !MOBTIMER_FINISHED(src, MT_FOUNDSNEAK, 30 SECONDS) || !T || reset || (m_intent != MOVE_INTENT_SNEAK) || light_amount >= sneaking_light_threshhold)
		...
		if(light_amount < sneaking_light_threshhold && m_intent == MOVE_INTENT_SNEAK)
